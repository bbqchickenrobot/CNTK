...
# some simple code for testing forward and backprop
"""x_shape = (64,3,3,3)
mean_bits = 4
x = input_variable(shape=x_shape, needs_gradient=True)
#bit_map = np.asarray(np.maximum(np.round(np.random.normal(mean_bits, 1, x_shape)), 1), dtype=np.int32)
#bit_map = mean_bits
out = CustomMultibitKernel(x, 1)
#out = CustomSign(x)
randvals = 2*(np.random.rand(64,3,3,3) - .5)
inval = np.asarray(randvals, dtype=np.float32)
result = out.eval({x: inval}, as_numpy=True)
print(result)

#print(np.mean(np.abs(result - inval)))
#grad = out.grad({x: inval}, as_numpy=True)
#print(grad)
"""
